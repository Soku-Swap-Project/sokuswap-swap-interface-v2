...
import { getChainIdByRubicName, getChainTypeByChainId } from 'connectors/helpers'
...
const Swap = ({ banners }: SwapProps) => {
  ...
  const [selectedFromChain, setSelectedFromChain] = useState<BlockchainName>(
    getChainTypeByChainId(chainId as number) ?? 'ETH'
  )
  const [selectedToChain, setSelectedToChain] = useState<BlockchainName>()

  const selectedTokenAChainId = getChainIdByRubicName(selectedFromChain as BlockchainName)
  const selectedTokenBChainId = getChainIdByRubicName(selectedToChain as BlockchainName)

  const loadedUrlParams = useDefaultsFromURLSearch(selectedTokenBChainId as number)
  ...
  const {
    ...
    allowedSlippage,
    to,
  } = useDerivedSwapInfo(selectedTokenAChainId as number, selectedTokenBChainId as number)
  ...
  //   parseFloat(currencyBalances[Field.INPUT]?.toExact() as any) < parseFloat(formattedAmounts[Field.INPUT])
  ...
  const impact = rubicTrade?.priceImpact || rubicTrade?.priceImpactData?.priceImpactTo

  // eslint-disable-next-line react-hooks/exhaustive-deps
  const priceImpact = new Percent(hasPriceImpact ? parseInt(impact) : 0)
  ...
  const estimatedRecievedAmount = crossChainTradeError ? 0 : rubicTrade?.to.tokenAmount
  ...
  const minAmountReceived = rubicTrade?.toTokenAmountMin

  const crossChainErrorMessage = crossChainTradeError?.message ? crossChainTradeError?.message : 'No Trade Available'
  ...
  useEffect(() => {
    if (selectedFromChain === selectedToChain || chainId === selectedTokenBChainId) {
      const inputToken = inputTokenWrapped?.isNative ? 'ETH' : inputTokenWrapped?.address
      const outputToken = outputTokenWrapped?.isNative ? 'ETH' : outputTokenWrapped?.address

      // router.push('/swap')
      router.replace(`/swap?inputCurrency=${inputToken}&outputCurrency=${outputToken}`)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    <div style={{ marginTop: '-20px' }}>
      ...
      <SwapLayoutCard>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <NetworkModel disabled={true} selectedChain={selectedFromChain} setSelectedChain={setSelectedFromChain} />
        </div>
        <div className="flex flex-col gap-3">
          ...
          <SwapAssetPanel
            ...
            selectedChain={selectedToChain ?? undefined}
          />
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <NetworkModel selectedChain={selectedToChain} setSelectedChain={setSelectedToChain} />
          </div>
          {sushiGuardEnabled && <SwapGasFeeInputs />}
          {isExpertMode && <RecipientField recipient={recipient} action={setRecipient} />}
          {Boolean(rubicTrade) && userHasSpecifiedInputOutput && !crossChainTradeError && (
            <SwapDetails
          ...
          {/* {rubicTrade && userHasSpecifiedInputOutput && (
          ...
          {swapIsUnsupported ? (
            ...
            <Button
              // color={isValid && priceImpactSeverity > 2 && !swapCallbackError ? 'red' : '#04bbfb'}
              fullWidth
              style={{
                marginTop: '12px',
            ...
            >
              {insufficientFunds
                ...
                ? 'No Trade Available'
                : !isLoading && rubicTrade && estimatedRecievedAmount
                ? i18n._(t`Swap`)
                : 'Enter an amount'}
            </Button>
          )}
