...
const AmountInput = ({ state }: { state: MigrateState }) => {
  ...
  if (!state.selectedLPToken) {
    return (
      <>
        <div className="p-3 text-center rounded cursor-not-allowed bg-light-800">
...
const LPTokenSelect = ({ lpToken, onToggle, isSelected, updating, exchange }: PositionCardProps) => {
  return (
    <div
      key={lpToken.address}
      className="flex items-center justify-between px-3 py-5 rounded cursor-pointer bg-light-800 hover:bg-dark-700"
