...
export function useMultipleContractSingleDataWithChainId(
  chainIdParam: number | undefined,
  ...args: SkipFirstTwoParams<typeof multicall.hooks.useMultipleContractSingleData>
) {
  const { chainId, latestBlock } = useCallContext()

  return multicall.hooks.useMultipleContractSingleData(chainIdParam ?? chainId, latestBlock, ...args)
}
...
export function useSingleContractMultipleDataWithChainId(
  chainIdParam: number | undefined,
  ...args: SkipFirstTwoParams<typeof multicall.hooks.useSingleContractMultipleData>
) {
  const { chainId, latestBlock } = useCallContext()
  return multicall.hooks.useSingleContractMultipleData(chainIdParam ?? chainId, latestBlock, ...args)
}
