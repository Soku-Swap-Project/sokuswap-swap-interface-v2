...
function useTokensFromMap(
  ...
  const { chainId: chainIdFromWeb3 } = useActiveWeb3React()

  const chainId = chainIdParam ?? chainIdFromWeb3
      ...
      return (
        userAddedTokens
          ...
          .reduce<{ [address: string]: Token }>(
            (tokenMapP, token) => {
              const tokenMap2 = tokenMapP
              tokenMap2[token.address] = token
              return tokenMap2
            },
...
export function useAllTokensByChainId(chainId: number): { [address: string]: Token } {
  const allTokens = useCombinedActiveList()
...
export function useToken(tokenAddress?: string | null, chainIdParam?: number): Token | undefined | null {
  const { chainId: chainIdFromWeb3 } = useActiveWeb3React()
  const chainId = chainIdParam ?? chainIdFromWeb3
  const tokens = useAllTokensByChainId(chainIdParam as number)
...
export function useCurrency(currencyId: string | undefined, chainIdParam?: number): Currency | null | undefined {
  const { chainId: chainIdFromWeb3 } = useActiveWeb3React()
  const chainId = chainIdParam ?? chainIdFromWeb3
