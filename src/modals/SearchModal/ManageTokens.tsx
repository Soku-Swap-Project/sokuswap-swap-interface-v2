...
const ManageTokens: FC = () => {
  ...
  const tokenList = useMemo(() => {
    return (
      chainId &&
      userAddedTokens.map((token) => (
        <div className="flex justify-between px-4 py-3 hover:bg-light-800/40" key={token.address}>
          ...
          <div className="flex items-center gap-2">
            <div
              className="flex items-center w-4 h-4 hover_shadow_icon cursor-pointer"
              onClick={() => removeToken(chainId, token.address)}
            >
              <Trash width={20} />
            </div>
            <ExternalLinkIcon
              className="flex items-center cursor-pointer min-w-4 hover_shadow_icon min-h-4"
    ...
    <>
      <input
        ...
        className={classNames(
          searchQuery !== '' && !isAddressSearch ? 'border-red' : 'border-light-800 focus:border-blue',
          'w-full bg-light-800 hover_shadow border border-blue focus:border-blue rounded placeholder-secondary font-bold text-base p-4 appearance-none'
        )}
        value={searchQuery}
        autoComplete="off"
      ...
      <HeadlessUiModal.BorderedContent className="flex flex-col text-blue bg-light hover_shadow !p-0">
        ...
        <div className="divide-y divide-light-800">{tokenList}</div>
