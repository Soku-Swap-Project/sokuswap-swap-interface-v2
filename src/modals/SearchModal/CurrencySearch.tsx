...
export function CurrencySearch({
  ...
  const allTokensByChainId = useAllTokensByChainId((selectedChainId as number) ?? null)
  ...
  if (router.route === '/cross-swap') {
    allTokens = allTokensByChainId
  }
  ...
  // @ts-ignore TYPE NEEDS FIXING
  const ether = useMemo(() => selectedChainId && NATIVE[selectedChainId], [selectedChainId])

  const filteredSortedTokensWithETH: any[] = useMemo(() => {
    const s = debouncedQuery.toLowerCase().trim()
    if (s === '' || s === 'e' || s === 'et' || s === 'eth') {
      return ether ? [getTokenFromCurrency(ether), ...filteredSortedTokens] : filteredSortedTokens
    }
  ...
  return (
    <>
      <h1 className="text-blue font-bold">Select a Token</h1>
      {showSearch && (
        <input
          type="text"
      ...
      <div className="h-full overflow-hidden overflow-y-auto border rounded border-light-800">
