...
import SOKU_TOKEN_LIST from 'constants/token-lists/sushiswap-default.tokenlist.json'
...
export function useCombinedActiveList(): TokenAddressMap {
  const activeListUrls = useActiveListUrls()
  const activeTokens = useCombinedTokenMapFromUrls(activeListUrls)
  const sokuTokens = listToTokenMap(SOKU_TOKEN_LIST)
  return useMemo(() => combineMaps(sokuTokens, activeTokens), [sokuTokens, activeTokens])
}
